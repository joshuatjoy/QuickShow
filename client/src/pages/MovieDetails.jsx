  ...
  return show ? (
    <div className='px-6 md:px-16 lg:px-40 pt-30 md:pt-50'>
          ...
          <div className='relative flex flex-col gap-3'>
            ...
            <div className='flex items-center flex-wrap gap-4 mt-4'>
              ...
              <button onClick={handleFavorite} className='bg-gray-700 p-2.5 rounded-full transition cursor-pointer active:scale-95'>
                <Heart className={`w-5 h-5 ${favoriteMovies.find(movie => movie._id === id) ? 'fill-primary' : ''}`}/>
              </button>
            </div>
          </div>
        </div>
      ...
      <div onClick={()=> {navigate('/movies'); scrollTo(0,0)}} className='flex justify-center mt-20'>
          <button className='px-10 py-3 text-sm bg-primary hover:bg-primary-dull transition rounded-md font-medium '>Show More</button>
      </div>
    </div>
  ) : <Loading />
}
